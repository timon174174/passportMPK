...
<?
$id_passport = $_GET[passport];

$query_passport = "SELECT * FROM passport WHERE id=$id_passport";

$result_passport = mysql_query($query_passport) or die(mysql_error());

$data_passport = mysql_fetch_array($result_passport);
		...
		<td>
			<select name="audience" id="">
				...
				while($data = mysql_fetch_array($result)){
					...
					if ($data[id]==$data_passport[id_audience]) {
					echo "<option value='$data[id]' selected>$data[number]</option>";
					}else{
	...
	<td>
		<select name="spr_t_cabinet_fgos" id="vent">
			...
			while($data = mysql_fetch_array($result)){
				...
				if ($data[id]==$data_passport[spr_t_cabinet_fgos]) {
...
<td>
	<select name="spr_ppssz" id="spr_ppssz">
		...
		while($data = mysql_fetch_array($result)){
			...
			if ($data[id_specialty]==$data_passport[spr_ppssz]) {
					echo "<option value='$data[id_specialty]' selected>$data[specialty] $data[name]</option>";
					}else{
...
<td>
	<select name="spr_department" id="">
		...
		while($data = mysql_fetch_array($result)){
			...
			if ($data[id]==$data_passport[spr_department]) {
					echo "<option value='$data[id]' selected>$data[id]$data[name]</option>";
					}else{
...
<div id="disciplines_div" style="margin-bottom: 10px;">
	<select name="disciplines" id="disciplines" multiple>
